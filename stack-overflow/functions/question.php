...
{
    $db = connectDB();
    $sql = "SELECT q.*, COUNT(a.id) answers_count FROM questions q LEFT JOIN answers a ON(q.id = a.question_id) WHERE q.title LIKE '%" . $search . "%' OR q.description LIKE '%" . $search . "%' GROUP BY q.id DESC LIMIT 10";
    $result = mysqli_query($db, $sql);
    $questions = array();
    while ($row = mysqli_fetch_array($result)) {
        $questions[] = $row;
